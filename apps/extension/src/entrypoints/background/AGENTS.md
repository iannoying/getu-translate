...
| `db-cleanup.ts`             | Three periodic alarms (`cache-cleanup`, `request-record-cleanup`, `summary-cache-cleanup`) plus exported `cleanupAll*` helpers for manual purges. Also clears the legacy `pdf-translations-eviction` alarm on every setup so old installs don't keep firing. |
