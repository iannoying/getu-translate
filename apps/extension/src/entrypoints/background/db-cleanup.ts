...
// Legacy alarm name from the in-extension PDF translation feature. The feature
// has been retired; we proactively cancel any leftover alarm on installs that
// upgraded across the deletion so the registry doesn't keep dispatching to
// nothing.
const LEGACY_PDF_TRANSLATIONS_EVICTION_ALARM = "pdf-translations-eviction"

export async function setUpDatabaseCleanup() {
  // Drop the legacy PDF-translations eviction alarm if a previous install
  // registered it. `clear` resolves false when no such alarm exists — safe.
  void browser.alarms.clear(LEGACY_PDF_TRANSLATIONS_EVICTION_ALARM)
