...
| `translate-current-pdf-button.tsx`     | Shortcut that opens the public web translator at `getutranslate.com/document/?src=<URL>` in a new tab. Shown only when the active tab URL ends with `.pdf`.                                               |
