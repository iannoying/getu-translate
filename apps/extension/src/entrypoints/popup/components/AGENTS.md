...
| `translate-current-pdf-button.tsx`     | Shortcut that opens the current tab's PDF URL inside the extension's bundled PDF viewer (`/pdf-viewer.html?src=...`). Shown only when the active tab is a `.pdf` navigation.                               |
| `review-entry-button.tsx`              | Opens the wordbook review page (SM-2) in a new tab. Displays a due-count badge from the local `words` Dexie table.                                                                                         |
