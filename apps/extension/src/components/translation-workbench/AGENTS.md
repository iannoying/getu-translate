...
Self-contained multi-provider translation workbench used by the sidebar (`side.content`) and translation-hub entrypoints. Orchestrates parallel translation requests across multiple configured providers, handles provider gating (anonymous / free / pro / enterprise plans), and renders per-provider result cards with language selection UI. Also hosts the **side-panel sidebar shell** (text + document tabs) introduced in M6.13 + the persistent-sidebar PR.
...
| `provider-gating.ts`        | `getProviderGate(provider, plan)` — pure plan + provider → gate mapping. Also exports `getTextTranslateCharLimit`, `isFreeTranslateProvider`, `isGetuProProvider`, `planFromEntitlements`, `buildSidebarClickRequestId`, `buildSidebarTokenRequestId`. |
...
| `sidebar-shell.tsx`         | Two-pane layout shell for the persistent sidebar: main content area + right-side vertical tablist (`text` / `document`). Manages `activeTab` local state and renders `<SidebarTextTab>` or `<SidebarDocumentTab>`. Receives `portalContainer` (for combobox/popover portals into the shadow root) and an `onClose` callback. Used inside `entrypoints/side.content`. |
| `sidebar-text-tab.tsx`      | Sidebar's text-translation pane. Wraps `WorkbenchLanguagePicker` + `ProviderMultiSelect` + `<TranslationWorkbenchResultCard>` per selected provider; persists provider selection at `SIDEBAR_SELECTED_PROVIDERS_STORAGE_KEY`; defaults to `getu-pro-gemini-3-flash-preview`. Drives `runTranslationWorkbenchRequest` on submit, gates plan via `useEntitlements` + `planFromEntitlements`, refreshes auth via `useAuthRefreshOnFocus`. Char-limit enforced via `getTextTranslateCharLimit(plan)`. |
| `sidebar-document-tab.tsx`  | Sidebar's document-translation pane. Static promotional surface that lists supported formats (PDF/EPUB/DOCX/TXT/HTML/Markdown/SRT/ASS/VTT/LRC) and three feature blurbs (PDF Pro layout, BabelDoc, subtitle files), with a CTA button that opens `WEB_DOCUMENT_TRANSLATE_URL` via the `openTab` background message. The actual document translation lives on `getutranslate.com/document/`; this tab is the redirect surface. |
