...
**`mode` 字段（v5 新增，替代 v4 的 `paymentMethod`）：**

- 可选，默认 `'subscription'`，向后兼容。
- `'subscription'`：使用 Stripe 订阅（`mode=subscription`，card-only）或 Paddle 订阅，自动续费。
- `'one_time'`：使用 Stripe 一次性支付（`mode=payment`，`automatic_payment_methods` 开启），不自动续费，到期需重新购买。Stripe 托管页面上自动显示已激活的支付方式。
- `mode=one_time` 且 `provider=paddle` 时返回 `BAD_REQUEST (400)`（Paddle 暂不支持一次性支付）。
